...
const createSnapshotsModule = ({
    ...
    const buildSnapshotAnalysis = () => {
        ...
        snapshotMetrics.value = [
            { key: 'latest', label: '最新总资产', value: formatMoney(last.total_assets) },
            { key: 'change', label: '区间总资产变化', value: formatMoney(totalChange, 2, true), color: totalChange >= 0 ? 'var(--app-up)' : 'var(--app-down)' },
            { key: 'profit', label: '投资盈亏变化', value: formatMoney(profitChange, 2, true), color: profitChange >= 0 ? 'var(--app-up)' : 'var(--app-down)' },
            { key: 'ratio', label: '当前投资 / 流动占比', value: `${investRatio.toFixed(1)}%` },
        ];
        const labels = {
            total_assets: '总资产',
